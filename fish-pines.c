...
// errp XX
    ...
    while (1) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            bool alt = *g.alt;
            int button_string_size = strlen(*button_name_ptr) + 1;
            if (alt) button_string_size += 6; // 'alt + '
            char *button_string = str(button_string_size);
            if (alt) 
                sprintf(button_string, "alt + %s", *button_name_ptr);
            else
